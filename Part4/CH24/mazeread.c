...
static void checkMalloc(void * ptr, char * message)
{
  if (ptr == NULL) { printf("malloc for %s fail\n", message); } 
}
static int findLineLength(FILE * fh)
{
  int ch;
  ...
  for (row = 0; row < numRow; row ++)
    {
      ...
      checkMalloc(mzptr -> cells[row], "mzptr -> cells[row]");
      // initialize the cells to invalid 
      for (col = 0; col < numCol; col ++)
	{ (mzptr -> cells)[row][col] = INVALIDSYMBOL; }
    }
  fseek(fptr, 0, SEEK_SET);   // move fptr to the beginning 
...
void Maze_destruct(Maze * mzptr) // release the memory 
{
  int row;
