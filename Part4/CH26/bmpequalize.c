...
void BMP_equalize(BMP_Image *img)
{
  ...
  // calculate the scaling factors, max and min must be different
  // to prevent divided by zero error
  double redscale   = 1.0;
  ...
  for (pxl = 0; pxl < (img -> data_size); pxl += 3)
	...
	{
	  img -> data[pxl + 2] =
	    (int) (redscale * (img -> data[pxl + 2] - redmin));
	}
      if (greenmax > greenmin)
	{
	  img -> data[pxl + 1] =
	    (int) (greenscale * (img -> data[pxl + 1] - greenmin));
	}
      if (bluemax > bluemin)
	{
	  img -> data[pxl] =
	    (int) (bluescale * (img -> data[pxl] - bluemin));
	}
    }
}
