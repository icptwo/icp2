...
//    argv[1]: the number of elements,  must be between 3 and 31
//    argv[2]: "1" the array is valid, "0" the array is invalid
//    argv[3]: "1" if there is a solution, "0" if there is no solution
...
void printArray(int * arr, int num, int kval)
{
  printf("%d\n", kval);
  int ind;
  for (ind = 0; ind < num; ind ++) {  printf("%d\n", arr[ind]); }
}
int main(int argc, char ** argv)
{
  ...
  if ((numInt < 3) || (numInt > 31))  { return EXIT_FAILURE; }
  if ((hasSol != 0) && (hasSol != 1)) { return EXIT_FAILURE; }
  if ((hasSol != 0) && (hasSol != 1)) { return EXIT_FAILURE; }
  ...
  if (isValid == 0)
    {
      if ((rand() % 2) == 0)
	{  arr[numInt - 1] = arr[0]; } // make two elements the same
      else
	{  arr[0] = - (rand() % 10000);	} // make an element negative or zero
	...
	{
	  if (hasSol == 0)
	    { kval += arr[ind] + 1; } // kval > sum of all elements
	  else
	    {
	      if ((rand() % 3) == 1) {  kval += arr[ind]; }
	    }
	}
      if (kval == 0) { kval = arr[0];} // only if hasSol is 1
    }
