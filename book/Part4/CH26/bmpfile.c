...
// return 0 if the header is invalid, 1 if the header is valid
...
BMP_Image *BMP_open(const char *filename)
{
  ...
  if ((img -> data) == NULL) { return cleanUp(fptr, img); } // malloc fail
  if (fread(img -> data, sizeof(char),
	    img -> data_size, fptr) != (img -> data_size))
    { return cleanUp(fptr, img); } // fread fails
  char onebyte;
  if (fread(& onebyte, sizeof(char), 1, fptr) != 0)
    {
      // the file still has data, something is wrong
      return cleanUp(fptr, img);
    }
